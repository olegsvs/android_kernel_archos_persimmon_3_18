...
extern void flash_gpio_output(int mode, int level);
...
static ssize_t FL_Init(void)
{
    ...
    flash_gpio_output(FLASH_GPIO_EN, 0);
...
static ssize_t FL_Enable(void)
{
	...
	{
		flash_gpio_output(FLASH_GPIO_EN, 0);
		flash_gpio_output(FLASH_GPIO_MODE, 0);
	}
	else if (g_duty == 0)
	{
		flash_gpio_output(FLASH_GPIO_EN, 1);
		flash_gpio_output(FLASH_GPIO_MODE, 0);
	}
	else
	{
		flash_gpio_output(FLASH_GPIO_EN, 1);
		flash_gpio_output(FLASH_GPIO_MODE, 1);
	}
	return 0;
