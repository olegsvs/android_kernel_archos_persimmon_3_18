...
#define POWER_NONE_MACRO -1 //MT65XX_POWER_NONE
...
//#define USE_EARLY_SUSPEND  //modify by  major  for can't sleep or auto reboot or display black when press power.
/*----------------------------------------------------------------------------*/
...
#define KXTJ2_INIT_SUCC    (0)
#define KXTJ2_INIT_FAIL    (-1)
/*----------------------------------------------------------------------------*/
...
static int    s_nInitFlag = KXTJ2_INIT_FAIL;
...
static DEFINE_MUTEX(kxtj2_1009_mutex_1);
...
static int KXTJ2_1009_WriteCalibration(struct i2c_client *client, int dat[KXTJ2_1009_AXES_NUM])
{
	...
	//added by nasri. Avoid to use wrong calibration data.
	if(obj->cali_sw[KXTJ2_1009_AXIS_X] > 180  || obj->cali_sw[KXTJ2_1009_AXIS_X] < -300 ||
		obj->cali_sw[KXTJ2_1009_AXIS_Y] > 180 || obj->cali_sw[KXTJ2_1009_AXIS_Y] < -180 ||
		obj->cali_sw[KXTJ2_1009_AXIS_Z] > 1500 || obj->cali_sw[KXTJ2_1009_AXIS_Z] < -1500)
	{
		obj->cali_sw[KXTJ2_1009_AXIS_X] = 0;
		obj->cali_sw[KXTJ2_1009_AXIS_Y] = 0;
		obj->cali_sw[KXTJ2_1009_AXIS_Z] = 0;
	}
#else
...
static void kxtj2_1009_early_suspend(struct early_suspend *h) 
{
	...
	if(err = KXTJ2_1009_SetPowerMode(obj->client, false))
	{
		GSE_ERR("early_suspend  write power control fail!!\n");
		mutex_unlock(&kxtj2_1009_mutex);
		return;
	}
	mutex_unlock(&kxtj2_1009_mutex);
...
static void kxtj2_1009_late_resume(struct early_suspend *h)
{
	...
	{
		GSE_ERR("late resume  initialize client fail!!\n");
		mutex_unlock(&kxtj2_1009_mutex);
		return;        
	}
	atomic_set(&obj->suspend, 0); 
...
static int kxtj2_1009_enable_nodata(int en)
{
    ...
    for(retry = 0; retry < 3; retry++){
        res = KXTJ2_1009_SetPowerMode(obj_i2c_data->client, power);
        if(res == 0)
        {
            GSE_LOG("KXTJ2_1009_SetPowerMode done\n");
            break;
        }
        GSE_LOG("KXTJ2_1009_SetPowerMode fail\n");
    }
...
static int kxtj2_1009_set_delay(u64 ns)
{
	...
	int sample_delay;
#endif//#ifdef CUSTOM_KERNEL_SENSORHUB
    printk("Susan add: set delay start \n");
	...
	if(value <= 5)
	{
	    printk("Susan add:set delay 200Hz \n");
		sample_delay = KXTJ2_1009_BW_200HZ;
	}
	else if(value <= 10)
	{
	    printk("Susan add:set delay 100Hz \n");
		sample_delay = KXTJ2_1009_BW_100HZ;
	}
	else
	{
	    printk("Susan add:set delay 50Hz \n");
		sample_delay = KXTJ2_1009_BW_50HZ;
	}

	mutex_lock(&kxtj2_1009_mutex);
	err = KXTJ2_1009_SetBWRate(obj_i2c_data->client, sample_delay);
	printk("Susan add: set delay OK \n");
	mutex_unlock(&kxtj2_1009_mutex);
	if(err != KXTJ2_1009_SUCCESS ) //0x2C->BW=100Hz
	{
		GSE_ERR("Set delay parameter error!\n");
...
static int  kxtj2_1009_local_init(void)
{
    ...
    if(KXTJ2_INIT_FAIL == s_nInitFlag)
    {
        return -1;
    }
