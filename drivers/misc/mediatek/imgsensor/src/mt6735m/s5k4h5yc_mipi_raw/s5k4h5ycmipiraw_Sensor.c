...
*/
//*********************************************//
static void S5K4H5_ReadOTPPage(int page, BYTE *Buff)  //read data from OTP;
{
	int i = 0;
	//Settting for 24 Mega MCLK.
	write_cmos_sensor(0x3b03,0x00);
	write_cmos_sensor(0x3b04,0x66);
	write_cmos_sensor(0x3b05,0x01);
	write_cmos_sensor(0x3b06,0xe6);
	write_cmos_sensor(0x3b07,0x06);
	write_cmos_sensor(0x3b08,0x08);
	write_cmos_sensor(0x3b0a,0x02);
	write_cmos_sensor(0x3b0b,0x76);
	write_cmos_sensor(0x3b0d,0x0a);
	write_cmos_sensor(0x3b0e,0x1d);
	write_cmos_sensor(0x3b0f,0x01);
	write_cmos_sensor(0x3b10,0xfd);
	// Set control register
	write_cmos_sensor(0x3a02,page);    //set page
	write_cmos_sensor(0x3a00,0x01);    //otp enable and read start
	//read otp data to buff
	for ( i=0 ; i < 64; i++ )
	{
		Buff[i] = read_cmos_sensor(0x3a04+i);
	}
	mdelay(10);
...
static  BOOL S5K4H5_ApplyWB(USHORT rg, USHORT bg, USHORT golden_rg, USHORT golden_bg)
{
	USHORT r_ratio=0, b_ratio=0;
	USHORT R_GAIN;
	USHORT B_GAIN;
	USHORT Gr_GAIN;
	USHORT Gb_GAIN;
	...
	LOG_INF("O-FILM OTP rg=%d ,bg=%d ,golden_rg=%d ,golden_bg=%d \n",rg,bg,golden_rg,golden_bg);
	...
	LOG_INF("O-FILM OTP RGAIN=%d ,BGAIN=%d ,GAIN=%d \n",R_GAIN,B_GAIN,G_GAIN);
...
//add by major for otp version check A

#define OFIM_S5K4H5YC_OTP_ID  0x07

int S5K4H5YC_OTP_ID= 0x00;
int S5K4H5YC_GET_OTP_ID(void)
{
     return S5K4H5YC_OTP_ID;
}
static int S5K4H5YC_otp_id(void)
{
	BYTE OTPData[64] = {0};
	int MID1=0;
	int MID2=0;

	S5K4H5_ReadOTPPage(0,OTPData);
	LOG_INF("O-FILM FLAG=%d \n",OTPData[19]);
	LOG_INF("O-FILM MID1=%d \n",OTPData[20]);
    LOG_INF("O-FILM MID2=%d \n",OTPData[1]);
	
	MID1 = OTPData[20];
	MID2 = OTPData[1];

	if (OFIM_S5K4H5YC_OTP_ID == MID1)
	{
		S5K4H5YC_OTP_ID = MID1;
		return MID1;
	}
	else if (OFIM_S5K4H5YC_OTP_ID == MID2)
	{
		S5K4H5YC_OTP_ID = MID2;
		return MID2;
	}else
	{
		S5K4H5YC_OTP_ID =0xFF;
		return 0xff;
	}
}
static void UpdateAwb(void)
{
	int RGain = 0, BGain = 0, RGain_Typical = 0, BGain_Typical = 0;
	BYTE OTPData[64] = {0};
	...
	LOG_INF("O-FILM MID1=%d \n",OTPData[20]);
    LOG_INF("O-FILM MID2=%d \n",OTPData[1]);
	if (OTPData[19] == 0) //group1
	{
	  RGain =  (OTPData[32]<<2) | (OTPData[36]>>6);
		BGain =  (OTPData[33]<<2) | ((OTPData[36]>>4)&0x03);
	  RGain_Typical = (OTPData[34]<<2) | ((OTPData[36]>>2)&0x03); //烧录的golden值
		BGain_Typical = (OTPData[35]<<2) | ((OTPData[36])&0x03);    //烧录的golden值
	}

	if (OTPData[19] == 0xff)//group2
	{
		RGain =  (OTPData[13]<<2) | (OTPData[17]>>6);
	  BGain =  (OTPData[14]<<2) | ((OTPData[17]>>4)&0x03);
		RGain_Typical = (OTPData[15]<<2) | ((OTPData[17]>>2)&0x03);
		BGain_Typical = (OTPData[16]<<2) | ((OTPData[17])&0x03);
	}

	if (!RGain||!BGain||!RGain_Typical||!BGain_Typical)
	{
		return;
	}
	S5K4H5_ApplyWB(RGain, BGain, RGain_Typical, BGain_Typical); 
}
//*********************************************//
...
{ 
  	...
  	write_cmos_sensor(0x0201,0x78);//Caval 140613
		...
		//********************************//
       write_cmos_sensor(0x3300,0x00);//OTP LSC 应用
	   UpdateAwb();  //OTP AWB 应用
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	//while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				...
				//add by major for otp version read
				 ofim_otp_id = S5K4H5YC_otp_id();
				 if (OFIM_S5K4H5YC_OTP_ID == ofim_otp_id)
				 {
				 	break;
				 }
				//add end
		...
		//retry = 2;
//	}
	if ((*sensor_id != imgsensor_info.sensor_id) ||(OFIM_S5K4H5YC_OTP_ID != ofim_otp_id) ) {
