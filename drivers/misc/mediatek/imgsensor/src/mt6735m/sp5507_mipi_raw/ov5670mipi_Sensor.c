...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	kal_uint8 retry = 2;
	int gpio2_value;
	printk("wilson sp5507 sibike get_imgsensor_id start\n");
	gpio2_value = gpio_get_value(2);
	printk("wilson++++++++++++ sp5507 sibike gpio2_value:%d, line:%d\n", gpio2_value,__LINE__);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if ((*sensor_id == imgsensor_info.sensor_id) && (gpio2_value == 0)){
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				printk("wilson sp5507 sibike i2c write id: 0x%x, sensor id: 0x%x, module is sibike module\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
			}
			LOG_INF("Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			printk("wilson sp5507 sibike Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
	...
	printk("wilson sp5507 sibike get_imgsensor_id end\n");
	return ERROR_NONE;
}
...
static kal_uint32 open(void)
{
	...
	printk("wilson sp5507 sibike open start\n");
	gpio2_value = gpio_get_value(2);
	printk("wilson++++++sp5507 sibike gpio2_value:%d, line:%d\n", gpio2_value,__LINE__);
	...
	if ((imgsensor_info.sensor_id != sensor_id) || (gpio2_value != 0))
		return ERROR_SENSOR_CONNECT_FAIL;
	...
	spin_unlock(&imgsensor_drv_lock);
	printk("wilson sp5507 sibike open end\n");
	return ERROR_NONE;
}	/*	open  */
