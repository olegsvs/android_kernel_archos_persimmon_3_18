...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	printk("wilson gc2365 sibike get_imgsensor_id start\n");
	gpio8_value = gpio_get_value(8);
	printk("wilson++++++++++gc2365 sibike gpio8_value:%d, line:%d\n", gpio8_value, __LINE__);
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            if ((*sensor_id == imgsensor_info.sensor_id) &&(gpio8_value == 0)){
				printk("wilson gc2365 i2c write id: 0x%x, sensor id: 0x%x, module is sibike module\n", imgsensor.i2c_write_id,*sensor_id);
                printk("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
			printk("wilson gc2365 sibike Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
	...
	int gpio8_value;
    LOG_1;
	printk("wilson+++++gc2365 sibike open start\n");
	gpio8_value = gpio_get_value(8);
	printk("wilson++++++++++++gc2365 sibike gpio8_value:%d, line:%d\n", gpio8_value,__LINE__);
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if ((sensor_id == imgsensor_info.sensor_id) && (gpio8_value == 0)) {
                printk("i2c write id: 0x%x, sensor id: 0x%x, moudle: sibike module\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            printk("Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			printk("wilson++++++++gc2365 sibike Read sensor id fail, write id: 0x%x, id: 0x%x, line:%d\n", imgsensor.i2c_write_id,sensor_id, __LINE__);
            retry--;
        } while(retry > 0);
        i++;
        if ((sensor_id == imgsensor_info.sensor_id) || (gpio8_value != 0))
	...
	printk("wilson+++++gc2365 sibike open end\n");
