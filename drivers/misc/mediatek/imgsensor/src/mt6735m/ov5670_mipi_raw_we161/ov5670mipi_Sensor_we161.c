...
#include "ov5670mipi_Sensor_we161.h"
...
#define RG_Ratio_Typical 0x138    //O-Film  offer  
#define BG_Ratio_Typical 0x15E   //O-Film  offer   


/********otp_struct  add bby o-film*******/
struct otp_struct {
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV5670MIPI_WE161_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
/**add start**/
//return value
//bit[7] 0: no otp infor, 1 valid otp infor
//bit[6] 0: no otp wb, 1 valid otp wb

#define OFIM_OV5670_OTP_ID  0x0b

int OV5670_OTP_ID= 0x00;
int OV5670_GET_OTP_ID(void)
{
	return OV5670_OTP_ID;
}
...
int read_otp_info(void)
{
	...
	int temp1;
	 ...
	 write_cmos_sensor(0x0100,0x01);// ; software reset
	 mDELAY(50);
	...
	LOG_INF("O-FILM 0V5670  addr=0x%x  otp_flag=0x%x \n",addr,otp_flag);
	if(addr != 0) {

		OV5670_OTP_ID = read_cmos_sensor( addr );
		/*
	...
	LOG_INF("O-FILM 0V5670 Module_ID=0x%x \n",OV5670_OTP_ID);
	//set 0x5002[3] to ¡°1¡±
	temp1 = read_cmos_sensor(0x5002);
	write_cmos_sensor(0x5002, (temp1 | 0x08));
...
int apply_otp(struct otp_struct *otp_ptr)
{
	...
	if ((*otp_ptr).flag & 0x40) 
		...
		//calculate G gain
		R_gain = (RG_Ratio_Typical*1024) / rg;
		B_gain = (BG_Ratio_Typical*1024) / bg;
		G_gain = 1024;
		
		if (R_gain < 1024 || B_gain < 1024)
		{
			if (R_gain < B_gain)
				Base_gain = R_gain;
			else
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				...
				//add by major for factory read OTP info 
				ofilm_otp_id = read_otp_info();
				if(OFIM_OV5670_OTP_ID == ofilm_otp_id)
				 {
	...
	if ((*sensor_id != imgsensor_info.sensor_id) || (OFIM_OV5670_OTP_ID != ofilm_otp_id) ) {
		// if Sensor ID is not correct, Must set *sensor_id to 0xFFFFFFFF 
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}
	printk("wilson ov5670 get_imgsensor_id end\n");
	return ERROR_NONE;
}
...
static kal_uint32 open(void)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	printk("wilson ov5670 syx open end\n");
	return ERROR_NONE;
}	/*	open  */
...
UINT32 OV5670_WE161_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
