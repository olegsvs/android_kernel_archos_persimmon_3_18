...
#define LOG_1 printk("GC5005,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/
...
void GC5005_MIPI_enable_OTP(kal_bool otp_state)
{
	...
	{ 
		...
		printk("Enter GC5005_MIPI_enable_OTP: Enable OTP!\n");		
	}
	else			
	{
		otp_en = otp_en & 0x7f;
		...
		printk("Enter GC5005_MIPI_enable_OTP: Disable OTP!\n");
	}
}
...
{	
	...
	printk("GC5005_MIPI_READ_OTP_DD:total_number = %d\n",total_number);
	for(i=0; i<total_number; i++)
	{
		...
		{//Read OTP
			...
			x = ((dd1&0x0f)<<8) + dd0;
			y = (dd2<<4) + ((dd1&0xf0)>>4);
			printk("GC5005_MIPI_READ_OTP_DD : type = %d , x = %d , y = %d \n",type,x,y);
		...
		{
			printk("GC5005_MIPI_READ_OTP_DD : check_id[%d] = %x,checkid error!!\n",i,check_flag);
		}
	}
...
//dd
	...
	{
	case 0x00:
		printk("GC5005_MIPI_OTP: OTP DD is Empty !!\n");
		break;
	case 0x01:
		GC5005_MIPI_WRITE_SRAM_DD();
		break;
	case 0x02:
		printk("GC5005_MIPI_OTP: OTP DD is Invalid !!\n");
		break;
	default :
		break;
	}
...
{	
	...
	if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
	{
		...
		printk("GC5005MIPI analogic gain 1x, GC5005MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		printk("GC5005MIPI analogic gain 1.469x , GC5005MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		printk("GC5005MIPI analogic gain 2.165x , GC5005MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_4<= iReg)&&(iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		printk("GC5005MIPI analogic gain 3.105x , GC5005MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_5<= iReg)&&(iReg < ANALOG_GAIN_6))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		printk("GC5005MIPI analogic gain 3.632x , GC5005MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_6<= iReg)&&(iReg < ANALOG_GAIN_7))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		printk("GC5005MIPI analogic gain 4.750x , GC5005MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_7<= iReg)&&(iReg < ANALOG_GAIN_8))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		printk("GC5005MIPI analogic gain 8.569x , GC5005MIPI add pregain = %d\n",temp);
	}
	else
	{                                                                                         
		...
		write_cmos_sensor(0xb1, temp>>6);                                                       
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);                                                
		printk("GC5005MIPI analogic gain 11.69x , GC5005MIPI add pregain = %d\n",temp);
	}                                                                                         
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	printk("wilson gc5005 get_imgsensor_id start\n");
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
				printk("wilson gc5005 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                printk("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			printk("wilson gc5005 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
