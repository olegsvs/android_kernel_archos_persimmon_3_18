...
#include "ov5670mipi_Sensor_syx.h"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV5670MIPI_SYX_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	printk("wilson ov5670 get_imgsensor_id start\n");
	gpio2_value = gpio_get_value(2);
	printk("wilson++++++++++++ ov5670 gpio2_value:%d, line:%d\n", gpio2_value,__LINE__);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = (((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C)) + 2);
			if ((*sensor_id == imgsensor_info.sensor_id) && (gpio2_value == 1)){
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				printk("wilson ov5670 i2c write id: 0x%x, sensor id: 0x%x, module is sanyingxin module\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
			}
			LOG_INF("Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			printk("wilson ov5670 Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	printk("wilson ov5670 syx open start\n");
	gpio2_value = gpio_get_value(2);
	printk("wilson++++++++++++ov5670 syx gpio2_value:%d, line:%d\n", gpio2_value,__LINE__);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			if ((sensor_id == imgsensor_info.sensor_id) && (gpio2_value == 1)){
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
				printk("wilson ov5670 syx i2c write id: 0x%x, sensor id: 0x%x, module is sanyingxin module\n", imgsensor.i2c_write_id,sensor_id);
				break;
			}
			LOG_INF("Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			printk("wilson ov5670 syx Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
	...
	if ((imgsensor_info.sensor_id != sensor_id) || (gpio2_value != 1))
		return ERROR_SENSOR_CONNECT_FAIL;
...
UINT32 OV5670_SYX_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
