...
#include "ov5648mipi_Sensor_Ofg.h"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV5648MIPI_SENSOR_ID_OFG,
...
//******************************//did by o-film
static void OV5648_ReadOTPBank(int nBank, kal_uint16 *data)
{ 
/*		 
2. If the result of step 1 is valid.
3. Clear OTP buffer:
Set 0x3D00~0x3D0F to '0'
4. Set OTP read mode
Set 0x3D84 to 0xc0
5. Select OTP bank and OTP end register
Set 0x3D85[4] to proper bank.
Set 0x3D86 to right OTP end address.
6. load OTP data into buffer:
Set 0x3D81[0] = 1
7. Delay is needed, then disable OTP read:
Set 0x3D81[0] = 0
8. Read out OTP data from the last valid group:
Get data from 0x3D00~0x3D0F
9. Select next bankd and go to step 2 if needed
10. Clear OTP buffer:
Set 0x3D00~0x3D0F to '0'
	*/
	int i = 0;
	...
	for(i = 0; i < 0x10; i++ )
	{
		write_cmos_sensor( 0x3d00+i,  0);
	} 
	write_cmos_sensor(0x3d84, 0xc0); 
	
	if( nBank == 0 )
	{ 
		write_cmos_sensor(0x3d85, 0 );  //Set 0x3D85[4] to proper bank.
		write_cmos_sensor(0x3d86, 0x0f );          //Set 0x3D86 to right OTP end address.
	}
	else
	{
		write_cmos_sensor(0x3d85, 0x10 );  //Set 0x3D85[4] to proper bank./
		write_cmos_sensor(0x3d86, 0x1f );          //Set 0x3D86 to right OTP end address.
	}
	
    //	Set 0x3D81[0] = 1
	Value2 = read_cmos_sensor(0x3d81);
	write_cmos_sensor(0x3d81, Value2|0X01); 
	
	Sleep(50);//20ms delay here.
	
	write_cmos_sensor(0x3d81, Value2&0XFE);
	
	for( i = 0; i < 0x10; i++ )
	{
		data[i] = read_cmos_sensor(0x3d00+i);
		Sleep(5);
	}  
...
static void OV5648_ReadOTPdata(kal_uint32 *data)
{
	int i = 0;
	int valueGroup = 0;
	kal_uint32 RGain = 0; 
	kal_uint32 BGain = 0; 
    kal_uint16 tempbuf[40] = {0};
	OV5648_ReadOTPBank( 0, tempbuf );
	OV5648_ReadOTPBank( 1, tempbuf+16 );

	//printk("wilson debug read mid:%d, line:%d\n", tempbuf[5], __LINE__);

    for (i = 0;i < 40; i++)
    {
		LOG_INF("O-FILM OTP OTPData[%d]=%d \n",i,tempbuf[i]);
    }
	...
	else if((tempbuf[14])==0x07)
	{
		valueGroup=2;
	}
	else if((tempbuf[0x17])==0x07)
	{
		valueGroup=3;
	}

	RGain =  (tempbuf[7+9*(valueGroup - 1)]<<2)|(tempbuf[11+9*(valueGroup - 1)]>>6);
	BGain =  (tempbuf[8+9*(valueGroup - 1)]<<2)|((tempbuf[11+9*(valueGroup - 1)]>>4)%4);
	LOG_INF("O-FILM OTP RGain=%d, BGain=%d,\n",RGain,BGain);
	printk("O-FILM OTP RGain=%d, BGain=%d,\n",RGain,BGain);
	data[0] = RGain;
	data[1] = BGain;
}


static kal_uint16 OV5648_OTP_combination_read(void)
{
	//int i = 0;
    kal_uint16 tempbuf[40] = {0};
...
//	OV5648_ReadOTPBank( 1, tempbuf+16 );
	...
	printk("wilson debug read ofg mid:%d, line:%d\n", tempbuf[5], __LINE__);

/*
	for (i = 0;i < 40; i++)
    {
		LOG_INF("O-FILM OTP OTPData[%d]=%d \n",i,tempbuf[i]);
		printk("wilson debug O-FILM OTP OTPData[%d]=%d \n",i,tempbuf[i]);
    };
*/
	if (7 == tempbuf[5])
		return TRUE;
...
#define  RG_Ratio_Typical  400
#define  BG_Ratio_Typical  400

static void  OV5648_ApplyWB(kal_uint32 rg,kal_uint32 bg)
{
	int R_Gain,B_Gain,G_Gain,G_Gain_R,G_Gain_B;
	...
	LOG_INF("O-FILM OTP RGain=%d, BGain=%d,G_Gain=%d\n",R_Gain,B_Gain,G_Gain);
	printk("O-FILM OTP RGain=%d, BGain=%d,G_Gain=%d\n",R_Gain,B_Gain,G_Gain);
	if (R_Gain>0x400) 
	{
...
static void OV5648_UpdateOTP(void)
{
    kal_uint32 OTPData[2] = {0};
    OV5648_ReadOTPdata(OTPData);
	OV5648_ApplyWB(OTPData[0],OTPData[1]);
}
//******************************//did by o-film
...
//    sensor_id = ((read_cmos_sensor(0x300A) << 8) | read_cmos_sensor(0x300B));
    //printk("wilson debug ofg sensor_id:0x%x, line:%d\n", sensor_id, __LINE__);
...
static kal_uint16 OV5648MIPI_check_otp_wb(kal_uint16 index)
{
    kal_uint16 i;
    ...
    if(index == 2)  //Group 2
    {
    	//read otp bank 1
        write_cmos_sensor(0x3d84,0xc0);	
		write_cmos_sensor(0x3d85,0x10); //OTP start address, bank 0
		write_cmos_sensor(0x3d86,0x1f); //OTP end address
	    write_cmos_sensor(0x3d81,0x01); //OTP Read Enable
        
		msleep(50); // delay 50ms
        
		rg = (read_cmos_sensor(0x3d07) << 8) + read_cmos_sensor(0x3d08);	
		bg = (read_cmos_sensor(0x3d09) << 8) + read_cmos_sensor(0x3d0a);
    }
    else if(index == 1) //Group 1
    {
    	//read otp bank 0
        write_cmos_sensor(0x3d84,0xc0);	
		write_cmos_sensor(0x3d85,0x00); //OTP start address, bank 0
		write_cmos_sensor(0x3d86,0x0f); //OTP end address
	    write_cmos_sensor(0x3d81,0x01); //OTP Read Enable
        
		msleep(50); // delay 50ms
        
		rg = (read_cmos_sensor(0x3d0a) << 8) + read_cmos_sensor(0x3d0b);	
		bg = (read_cmos_sensor(0x3d0c) << 8) + read_cmos_sensor(0x3d0d); 	   	
    }
	else
	{
        LOG_INF("invalid index!");
	printk("ofg invalid index!");
        return FALSE;
	}
...
//return:	0.
static kal_uint16 OV5648MIPI_read_otp_wb(kal_uint16 index, struct OV5648MIPI_otp_struct *otp_ptr)
{
	kal_uint16 i;
    ...
    if(index == 2)  //Group 2
    {
        ...
        (*otp_ptr).rg_ratio = (read_cmos_sensor(0x3d07) << 8) + read_cmos_sensor(0x3d08);	
		(*otp_ptr).bg_ratio = (read_cmos_sensor(0x3d09) << 8) + read_cmos_sensor(0x3d0a);
        (*otp_ptr).gb_gr_ratio = (read_cmos_sensor(0x3d0b) << 8) + read_cmos_sensor(0x3d0c);
    }
    else if(index == 1) //Group 1
    {
    	//read otp bank 0
        ...
        (*otp_ptr).rg_ratio = (read_cmos_sensor(0x3d0a) << 8) + read_cmos_sensor(0x3d0b);	
		(*otp_ptr).bg_ratio = (read_cmos_sensor(0x3d0c) << 8) + read_cmos_sensor(0x3d0d);
        (*otp_ptr).gb_gr_ratio = (read_cmos_sensor(0x3d0e) << 8) + read_cmos_sensor(0x3d0f); 	
    }
	else
	{
        LOG_INF("invalid index!");
    ...
    LOG_INF("Read wb otp data success!");
    LOG_INF("rg_ratio = %d, bg_ratio = %d, gb_gr_ratio = %d", (*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio, (*otp_ptr).gb_gr_ratio);
    printk("ofg Read wb otp data success!");
    printk("ofg rg_ratio = %d, bg_ratio = %d, gb_gr_ratio = %d", (*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio, (*otp_ptr).gb_gr_ratio);
...
//reutrn 0
static kal_uint16 OV5648MIPI_update_wb_gain(kal_uint32 R_gain, kal_uint32 G_gain, kal_uint32 B_gain)
{
    LOG_INF("R_gain[%x] G_gain[%x] B_gain[%x]", R_gain, G_gain, B_gain);
...
static kal_uint32 RG_Ratio_Typical = RG_Typical;
static kal_uint32 BG_Ratio_Typical = BG_Typical;
static kal_uint32 GB_GR_Ratio_Typical = GB_GR_Typical;


//call this function after OV5648 initialization
//return value:	0: Update success
//			      1: No OTP
static kal_uint16 update_otp(void)
{	
	...
	{  
		//no valid wb OTP data
	 	LOG_INF("no valid wb OTP data!");
		printk("ofg no valid wb OTP data!");
		return 1;
	}
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
			printk("wilson debug ofg sensor_id: %d, line:%d\n", *sensor_id, __LINE__);
            if ((*sensor_id == imgsensor_info.sensor_id) && (OV5648_OTP_combination_read() == TRUE)){
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				//printk("wilson debug ofg i2c write id: 0x%x, sensor id: 0x%x, line:%d\n", imgsensor.i2c_write_id,*sensor_id, __LINE__);
                return ERROR_NONE;
            }
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
			//gpio_status = mt_get_gpio_pull_enable(GPIO42);
			//printk("wilson debug gpio_status:%d, line:%d\n", gpio_status, __LINE__);
            if ((sensor_id == imgsensor_info.sensor_id)/* && (OV5648_OTP_combination_read() == TRUE)*/) {
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
				//printk("wilson debug ofg i2c write id: 0x%x, sensor id: 0x%x, line:%d\n", imgsensor.i2c_write_id,sensor_id, __LINE__);
                break;
            ...
            LOG_INF("Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			//printk("wilson debug Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
    ...
    if ((imgsensor_info.sensor_id != sensor_id) /*|| (OV5648_OTP_combination_read() != TRUE)*/)
        return ERROR_SENSOR_CONNECT_FAIL;

    /* initail sequence write in  */
    sensor_init();

    mDELAY(5);  //add by darren
    OV5648_UpdateOTP(); //add by darren
...
UINT32 OV5648MIPISensorInit_Ofg(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
