...
*
********************************************************************************/
#define CAM_CAL_DRVNAME "S5K3M2_QT_CAM_CAL_DRV"
#define CAM_CAL_CLASS_NAME "S5K3M2_QT_CAM_CALdrv"
...
typedef struct {
	...
	u8 ModuleID;//offset 0x0005
	u8 LensId;//offset 0x0006
	u16 AwbAfInfo;
	u8 UnitAwbR;//offset 0x0009
	u8 UnitAwbGr;//offset 0x000A
	u8 UnitAwbGb;//offset 0x000B
	u8 UnitAwbB;
	u8 GoldenAwbR;
	u8 GoldenAwbGr;
	u8 GoldenAwbGb;
	u8 GoldenAwbB;
	u16 Reserve;//offset 0x0011
	u16 LscTableSize;//offset 0x0015
	u8 Lsc[MAX_LSC_SIZE];
}OTP_OFILM_TYPE;

typedef struct {
	u8 data[MAX_OTP_SIZE];
//	OTP_OFILM_TYPE OfilmOtpData;
}OTP_DATA;

static OTP_DATA darren_s5k3m2_eeprom_data = {{0}};

static int otp_flag=0;
static int s5k3m2_eeprom_read=0;

extern bool read_3m2_qt_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size);

int read_s5k3m2_qt_eeprom_mtk_fmt(void)
{
	...
	//read flag
	read_3m2_qt_eeprom(0x0000, &darren_s5k3m2_eeprom_data.data[0x00], 1);
	//read module info
	read_3m2_qt_eeprom(0x0001, &darren_s5k3m2_eeprom_data.data[0x01], 9);

	//read awb
	read_3m2_qt_eeprom(0x001C, &darren_s5k3m2_eeprom_data.data[0x1C], 13);
	


	//read lsc flag
	read_3m2_qt_eeprom(0x003A, &darren_s5k3m2_eeprom_data.data[0x3A], 1);
	//read lsc table
	read_3m2_qt_eeprom(0x003B, &darren_s5k3m2_eeprom_data.data[0x3B], 1868);

	//read afflag
	read_3m2_qt_eeprom(0x0788, &darren_s5k3m2_eeprom_data.data[0x788], 1);
	//read afinfo
	read_3m2_qt_eeprom(0x0789, &darren_s5k3m2_eeprom_data.data[0x789], 1);
	
	//reaf afinf
	read_3m2_qt_eeprom(0x078A, &darren_s5k3m2_eeprom_data.data[0x78A], 2);
	//reaf afmacro
	read_3m2_qt_eeprom(0x078C, &darren_s5k3m2_eeprom_data.data[0x78C], 2);
	
	return 1;
}

int selective_qt_read_region(u32 offset, u32 size, BYTE* data)
{
	memcpy((void*)data,(void*)&darren_s5k3m2_eeprom_data.data[offset],size);
	CAM_CALDB("selective_qt_read_region offset = %x, size:%d,data read %d\n",offset,size,*data);
	return size;
}
...
#if 1
    err |=get_user(p, (__u16 __user *)&data->pu1Params);
...
#endif
{
    ...
    CAM_CALDB("[CAM_CAL] CAM_CAL_Ioctl \n");
...
//            i4RetValue = iReadData((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
			i4RetValue = selective_qt_read_region(ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
...
static int CAM_CAL_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    spin_unlock(&g_CAM_CALLock);
    CAM_CALDB("Darren CAM_CAL_Open ok\n");
    return 0;
}
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    CAM_CAL_class = class_create(THIS_MODULE, CAM_CAL_CLASS_NAME);
