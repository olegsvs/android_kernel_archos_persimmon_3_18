...
#include "s5k3m2mipi_qt_Sensor.h"

#define DARREN_CAM_CAL_TEST
#ifdef DARREN_CAM_CAL_TEST
extern int read_s5k3m2_qt_eeprom_mtk_fmt(void);
#endif
...
#define OFILM_QT_COMP

#ifdef OFILM_QT_COMP
#define QT_MODULE 0x06
#define OFILM_MODULE 0x0A
static int module_flag = 0;
#endif

static DEFINE_SPINLOCK(imgsensor_drv_lock);
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K3M2_QT_SENSOR_ID,
...
/* add by darren start */
//#define S5K3M2_OTP_AWB_CALIBRATION 1
#ifdef S5K3M2_OTP_AWB_CALIBRATION 

//#define USHORT             unsigned short
//#define BYTE               unsigned char
#define GAIN_DEFAULT  256
static int S5K3M2_applyWB(USHORT rg, USHORT bg, USHORT golden_rg, USHORT golden_bg)
{
	USHORT r_ratio, b_ratio;
	USHORT R_GAIN, B_GAIN, Gr_GAIN, Gb_GAIN,G_GAIN;
	
	r_ratio = (USHORT)(512 * (golden_rg) / (rg));
	b_ratio = (USHORT)(512 * (golden_bg) / (bg));
	...
	printk("Darren R_GAIN: %d,G_GAIN:%d,B_GAIN:%d\n",R_GAIN,G_GAIN,B_GAIN);
	write_cmos_sensor(0x6028, 0x4000);
	write_cmos_sensor(0x602A, 0x0100);
	write_cmos_sensor_8(0x6F12, 0x00); //stream off
	mdelay(20);
	write_cmos_sensor(0x6028, 0x4000);
	...
	write_cmos_sensor(0x6028, 0x4000);
	write_cmos_sensor(0x602A, 0x0100);
	write_cmos_sensor_8(0x6F12, 0x01); //stream on
	return 1;
}
static int otp_wb_update(void)
{
	
	BYTE OTP_R,OTP_Gr,OTP_Gb,OTP_B;
	BYTE Golden_R, Golden_Gr;
	BYTE Golden_Gb, Golden_B;
        BYTE temp;
#if 0
	BYTE readbuff;
	read_3m2_qt_eeprom(0x0009,&readbuff,1);
	OTP_R = readbuff;
	read_3m2_qt_eeprom(0x000A,&readbuff,1);
	OTP_Gr = readbuff;
	read_3m2_qt_eeprom(0x000B,&readbuff,1);
	OTP_Gb = readbuff;
	read_3m2_qt_eeprom(0x000C,&readbuff,1);
	OTP_B = readbuff;
	read_3m2_qt_eeprom(0x000D,&readbuff,1);
	Golden_R = readbuff;
	read_3m2_qt_eeprom(0x000E,&readbuff,1);
	Golden_Gr = readbuff;
	read_3m2_qt_eeprom(0x000F,&readbuff,1);
	Golden_Gb = readbuff;
	read_3m2_qt_eeprom(0x0010,&readbuff,1);
	Golden_B = readbuff;
#else
read_3m2_qt_eeprom(0x0000,&temp,1);
	read_3m2_qt_eeprom(0x0009,&OTP_R,1);
	read_3m2_qt_eeprom(0x000A,&OTP_Gr,1);
	read_3m2_qt_eeprom(0x000B,&OTP_Gb,1);
	read_3m2_qt_eeprom(0x000C,&OTP_B,1);
	read_3m2_qt_eeprom(0x000D,&Golden_R,1);
	read_3m2_qt_eeprom(0x000E,&Golden_Gr,1);
	read_3m2_qt_eeprom(0x000F,&Golden_Gb,1);
	read_3m2_qt_eeprom(0x0010,&Golden_B,1);
#endif

#if 0
	USHORT RG_fin = (USHORT)(512.0 * OTP_R / OTP_Gb + 0.5);
	USHORT BG_fin = (USHORT)(512.0 * OTP_B / OTP_Gb + 0.5);
	
	USHORT Typical_RG = (USHORT)(512.0 * Golden_R / Golden_Gb + 0.5);
	USHORT Typical_BG = (USHORT)(512.0 * Golden_B / Golden_Gb + 0.5);
#else
	USHORT RG_fin = (USHORT)(512 * OTP_R / OTP_Gb );
	USHORT BG_fin = (USHORT)(512 * OTP_B / OTP_Gb );
	
	USHORT Typical_RG = (USHORT)(512 * Golden_R / Golden_Gb);
	USHORT Typical_BG = (USHORT)(512 * Golden_B / Golden_Gb);
#endif
	
	S5K3M2_applyWB(RG_fin, BG_fin,Typical_RG, Typical_BG);
	printk("Darren 0x0000 temp:%d\n",temp);
	printk("Darren OTP_R:%d, OTP_Gr:%d, OTP_Gb:%d, OTP_B:%d\n",OTP_R,OTP_Gr,OTP_Gb,OTP_B);
	printk("Darren Golden_R:%d, Golden_Gr:%d, Golden_Gb:%d, Golden_B:%d\n",Golden_R,Golden_Gr,Golden_Gb,Golden_B);
	printk("Darren RG_fin:%d, BG_fin:%d\n",RG_fin,BG_fin);
	printk("Darren Typical_RG:%d, Typical_GB:%d\n",Typical_RG,Typical_BG);
	return 1;
}
#endif
/* add by darren end */
...
mdelay(100); //200 to 100 by darren
...
static kal_uint32 check_otp_id(void)
{
	unsigned char readbuff = 0x0;
	
	/* check otp flag*/
	selective_qt_read_region(0x0000, 1, &readbuff);
//	read_3m2_qt_eeprom(0x0000, &readbuff, 1);
	if(readbuff != 0x01)
	{
		LOG_INF("module otp flag must be 0x01! but now %x\n", readbuff);
		return 0;
	}
	selective_qt_read_region(0x0001, 1, &readbuff);
//	read_3m2_qt_eeprom(0x0001, &readbuff, 1);
	if(readbuff == 0x06 ) // may QT module
	{
		/* it may be qt module */
		LOG_INF("HAHA it may QT module, make sure it\n");
#if 0
		read_3m2_qt_eeprom(0x0006, &readbuff, 1); // read sensor id
		if(readbuff == 0x31)
		{
			LOG_INF("It must be QT module\n");
			module_flag = QT_MODULE;
			return QT_MODULE;
		}
		else
		{
			LOG_INF("check sensor id fail, it not QT module\n");
			return 0;
		}
#else
		module_flag = QT_MODULE;
		LOG_INF("It must be QT module: %d\n", module_flag);
		return QT_MODULE;
#endif
	}
	else if(readbuff == 0xff) // may ofilm module
	{
		/* it may be ofilm module */
		LOG_INF("HAHA it may ofilm module, make sure it\n");
		selective_qt_read_region(0x0005, 1, &readbuff);
//		read_3m2_qt_eeprom(0x0005, &readbuff, 1); // read module id
		if(readbuff == OFILM_MODULE)
		{
			LOG_INF("It must be OFILM module\n");
			module_flag = OFILM_MODULE;
			return OFILM_MODULE;
		}
		else
		{
			LOG_INF("check module id fail, it not OFILM module\n");
			return 0;
		}
	}
	LOG_INF("check otp id error\n");
	return 0;
}
#endif
/*************************************************************************
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12) + QT_MODULE;
			//*sensor_id = imgsensor_info.sensor_id;
            if (*sensor_id == imgsensor_info.sensor_id) {               
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			
				#ifdef DARREN_CAM_CAL_TEST
				read_s5k3m2_qt_eeprom_mtk_fmt();
				#endif
...
#ifdef OFILM_QT_COMP
    /* add by darren for otp compat */
    // 0x0a for ofilm
    // 0x06 for qt
    otp_id = check_otp_id();
    LOG_INF("read sensor ID:%d\n", otp_id);
    if(otp_id != QT_MODULE)
    {
    	LOG_INF("It's not QT module\n");
...
#if 0//def OFILM_QT_COMP
	    //check otp
	    otp_id = check_otp_id();
	    LOG_INF("Darren otp_id :%x\n",otp_id);
	    if(otp_id != OFILM_MODULE)
	    {
	    	LOG_INF("It's not OFILM module\n");
	    	sensor_id = 0xFFFFFFFF;
	    }
#endif
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:	
			...
			read_3m2_qt_pdaf((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;			
...
UINT32 S5K3M2_QT_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
