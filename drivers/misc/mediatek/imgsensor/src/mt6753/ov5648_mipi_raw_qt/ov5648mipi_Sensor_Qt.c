...
#include "ov5648mipi_Sensor_Qt.h"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV5648MIPI_SENSOR_ID_QT,
...
static void OV5648_ReadOTPdata(kal_uint32 *data)
{
	...
	else if((tempbuf[14])==0x06)
	{
		valueGroup=2;
	}
	else if((tempbuf[0x17])==0x06)
	{
		valueGroup=3;
	}
...
static kal_uint16 OV5648_OTP_combination_read(void)
{
	...
	printk("wilson debug read qt mid:%d, line:%d\n", tempbuf[5], __LINE__);

/*
	for (i = 0;i < 40; i++)
    {
		LOG_INF("QT OTP OTPData[%d]=%d \n",i,tempbuf[i]);
		printk("wilson debug QT OTP OTPData[%d]=%d \n",i,tempbuf[i]);
    };
*/
	if (6 == tempbuf[5])
		return TRUE;
...
static void  OV5648_ApplyWB(kal_uint32 rg,kal_uint32 bg)
{
	...
	printk("QT OTP RGain=%d, BGain=%d,G_Gain=%d\n",R_Gain,B_Gain,G_Gain);
	if (R_Gain>0x400) 
	{
...
//    sensor_id = ((read_cmos_sensor(0x300A) << 8) | read_cmos_sensor(0x300B));
    //printk("wilson debug qt sensor_id:0x%x, line:%d\n", sensor_id, __LINE__);
//    sensor_id += 1;
    //printk("wilson debug qt sensor_id:0x%x, line:%d\n", sensor_id, __LINE__);
...
static kal_uint16 OV5648MIPI_read_otp_wb(kal_uint16 index, struct OV5648MIPI_otp_struct *otp_ptr)
{
    ...
    printk("qt Read wb otp data success!");
    printk("qt rg_ratio = %d, bg_ratio = %d, gb_gr_ratio = %d", (*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio, (*otp_ptr).gb_gr_ratio);
...
{	
	...
	{  
		...
		printk("qt no valid wb OTP data!");
		return 1;
	}
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            LOG_INF("Read sensor id fail, write id:0x%x id: 0x%x, line:%d\n", imgsensor.i2c_write_id,sensor_id, __LINE__);
			//printk("wilson debug Read sensor id fail, write id:0x%x id: 0x%x, line:%d\n", imgsensor.i2c_write_id,sensor_id, __LINE__);
            retry--;
        } while(retry > 0);
        i++;
...
UINT32 OV5648MIPISensorInit_Qt(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
