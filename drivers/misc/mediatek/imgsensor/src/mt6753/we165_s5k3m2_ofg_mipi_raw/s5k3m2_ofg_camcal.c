...
*
********************************************************************************/
#define CAM_CAL_DRVNAME "S5K3M2_OFG_CAM_CAL_DRV"
#define CAM_CAL_CLASS_NAME "S5K3M2_OFG_CAM_CALdrv"
...
extern bool read_3m2_ofg_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size);

int read_s5k3m2_ofg_eeprom_mtk_fmt(void)
{
	...
	// for ofg
	//read flag
	read_3m2_ofg_eeprom(0x0000, &darren_s5k3m2_eeprom_data.data[0x00], 1);
	//read module info
	read_3m2_ofg_eeprom(0x0001, &darren_s5k3m2_eeprom_data.data[0x01], 8);
	
	//read awb
	read_3m2_ofg_eeprom(0x0009, &darren_s5k3m2_eeprom_data.data[0x09], 8);
	
	//read afinf
	read_3m2_ofg_eeprom(0x0011, &darren_s5k3m2_eeprom_data.data[0x11], 2);
	//reaf afmacro
	read_3m2_ofg_eeprom(0x0013, &darren_s5k3m2_eeprom_data.data[0x13], 2);
	//read lsc size
	read_3m2_ofg_eeprom(0x0015, &darren_s5k3m2_eeprom_data.data[0x15], 2);
	
	//read lsc table
	read_3m2_ofg_eeprom(0x0017, &darren_s5k3m2_eeprom_data.data[0x17], 1868);
	
	return 1;
}

int selective_ofg_read_region(u32 offset, u32 size, BYTE* data)
{
	memcpy((void*)data,(void*)&darren_s5k3m2_eeprom_data.data[offset],size);
	CAM_CALDB("selective_ofg_read_region offset = %x, size:%d,data read %d\n",offset,size,*data);
	return size;
}
...
//            i4RetValue = iReadData((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
			i4RetValue = selective_ofg_read_region(ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
