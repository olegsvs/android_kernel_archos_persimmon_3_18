...
#include "s5k3m2mipi_ofg_Sensor.h"

#define DARREN_CAM_CAL_TEST
#ifdef DARREN_CAM_CAL_TEST
extern int read_s5k3m2_ofg_eeprom_mtk_fmt(void);
#endif
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K3M2_OFG_SENSOR_ID,
...
static int otp_wb_update(void)
{
	...
	selective_ofg_read_region(0x0009, 1, &OTP_R);
	selective_ofg_read_region(0x000A, 1, &OTP_Gr);
	selective_ofg_read_region(0x000B, 1, &OTP_Gb);
	selective_ofg_read_region(0x000C, 1, &OTP_B);
	selective_ofg_read_region(0x000D, 1, &Golden_R);
	selective_ofg_read_region(0x000E, 1, &Golden_Gr);
	selective_ofg_read_region(0x000F, 1, &Golden_Gb);
	selective_ofg_read_region(0x0010, 1, &Golden_B);


	RG_fin = (USHORT)(512 * OTP_R / OTP_Gb );
	BG_fin = (USHORT)(512 * OTP_B / OTP_Gb );
	
	Typical_RG = (USHORT)(512 * Golden_R / Golden_Gb);
	Typical_BG = (USHORT)(512 * Golden_B / Golden_Gb);
...
static kal_uint32 check_otp_id(void)
{
	...
	/* check otp flag*/
	selective_ofg_read_region(0x0000, 1, &readbuff);
//	read_3m2_ofg_eeprom(0x0000, &readbuff, 1);
	if(readbuff != 0x01)
	{
		LOG_INF("module otp flag must be 0x01! but now %x\n", readbuff);
		return 0;
	}
	selective_ofg_read_region(0x0001, 1, &readbuff);
//	read_3m2_ofg_eeprom(0x0001, &readbuff, 1);
	if(readbuff == 0x06 ) // may QT module
	{
		/* it may be ofg module */
		LOG_INF("HAHA it may QT module, make sure it\n");
#if 0
		read_3m2_ofg_eeprom(0x0006, &readbuff, 1); // read sensor id
		if(readbuff == 0x31)
		{
	...
	else if(readbuff == 0xff) // may ofilm module
	{
		...
		selective_ofg_read_region(0x0005, 1, &readbuff);
//		read_3m2_ofg_eeprom(0x0005, &readbuff, 1); // read module id
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12) + OFILM_MODULE;
			//*sensor_id = imgsensor_info.sensor_id;
            if (*sensor_id == imgsensor_info.sensor_id) {               
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			
				#ifdef DARREN_CAM_CAL_TEST
				read_s5k3m2_ofg_eeprom_mtk_fmt();
				#endif
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:	
			...
			read_3m2_ofg_pdaf((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;			
...
UINT32 S5K3M2_OFG_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	s5k3m2_OFG_MIPI_RAW_SensorInit	*/
