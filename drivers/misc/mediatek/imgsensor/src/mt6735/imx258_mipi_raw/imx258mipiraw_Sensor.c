...
#define IMX258_OTP_AWB_CALIBRATION  1

#ifdef IMX258_OTP_AWB_CALIBRATION
...
extern bool read_imx258_eeprom_pdaf( kal_uint16 addr, BYTE* data, kal_uint32 size);
...
static void load_imx258_SPC_Data(void)
{
	...
	{
		if(!read_imx258_eeprom_SPC(0x0CB9,IMX258MIPI_SPC_Data,126))
		{
...
/* add by darren start */
#ifdef IMX258_OTP_AWB_CALIBRATION 
int imx258_applyWB(USHORT rg, USHORT bg)
{
	USHORT r_ratio, b_ratio;
	USHORT R_GAIN, B_GAIN, Gr_GAIN, Gb_GAIN,G_GAIN;
	
	r_ratio = (USHORT)(512 * (RG_Ratio_Typical) / (rg));
	b_ratio = (USHORT)(512 * (BG_Ratio_Typical) / (bg));
	...
	LOG_INF("OFILM R_GAIN: %d,G_GAIN:%d,B_GAIN:%d\n",R_GAIN,G_GAIN,B_GAIN);
...
static int otp_wb_update(void)
{
	...
	selective_ofg_read_region(0x0012, 1, &OTP_R);
	selective_ofg_read_region(0x0013, 1, &OTP_Gr);
	selective_ofg_read_region(0x0014, 1, &OTP_Gb);
	selective_ofg_read_region(0x0015, 1, &OTP_B);
	selective_ofg_read_region(0x0016, 1, &Golden_R);
	selective_ofg_read_region(0x0017, 1, &Golden_Gr);
	selective_ofg_read_region(0x0018, 1, &Golden_Gb);
	selective_ofg_read_region(0x0019, 1, &Golden_B);
	...
	imx258_applyWB(RG_fin, BG_fin);
	
	LOG_INF("OFILM OTP_R:%d, OTP_Gr:%d, OTP_Gb:%d, OTP_B:%d\n",OTP_R,OTP_Gr,OTP_Gb,OTP_B);
	LOG_INF("OFILM Golden_R:%d, Golden_Gr:%d, Golden_Gb:%d, Golden_B:%d\n",Golden_R,Golden_Gr,Golden_Gb,Golden_B);
	LOG_INF("OFILM RG_fin:%d, BG_fin:%d\n",RG_fin,BG_fin);
//	LOG_INF("OFILM Typical_RG:%d, Typical_GB:%d\n",Typical_RG,Typical_BG);
	
	return 1;
}

static bool check_otp_data(void)
{
    otp_wb_update();
...
#ifdef IMX258_OTP_AWB_CALIBRATION
				read_imx258_ofg_eeprom_mtk_fmt();
#endif
				return ERROR_NONE;
			}
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	sensor_info->PDAF_Support = 1;//add by darren
...
//	LOG_INF("feature_id = %d\n", feature_id);
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			...
			read_imx258_eeprom_pdaf((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
