...
//add by darren begin for eepom read
static kal_uint16 read_cmos_sensor_28(kal_uint32 addr)
{
	kal_uint16 get_byte=0;
	char puSendCmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF)};
		
	kdSetI2CSpeed(imgsensor_info.i2c_speed);
	iReadRegI2C(puSendCmd, 2, (u8*)&get_byte, 1, 0x28);
		
	return get_byte&0x00ff;
}
//add by darren end for eepom read
...
#define tRG_Ratio_typical    672   //0X2F0
#define tBG_Ratio_typical    662   //0X259

struct S5K3L2_MIPI_otp_struct
{
	BYTE AWB_Flag;
  BYTE MID;
  BYTE Lens_id;
...
struct S5K3L2_MIPI_otp_struct S5K3L2_OTP_Infor;
struct S5K3L2_MIPI_otp_struct * S5K3L2_OTP_Infor_Ptr = &S5K3L2_OTP_Infor;  

bool S5K3L2_get_otp_date(BYTE Page) 
{
	BYTE MID  = 0, Lens_id = 0;
	BYTE Year  = 0, Month = 0, Day   = 0;
	BYTE awb_rg_msb  = 0, awb_rg_lsb = 0, awb_bg_msb   = 0, awb_bg_lsb = 0;
	BYTE awb_GbGr_msb  = 0, awb_GbGr_lsb = 0;

	MID  = read_cmos_sensor_28(0x0001);
	Lens_id = read_cmos_sensor_28(0x0002);
	Year = read_cmos_sensor_28(0x0003);
	Month  = read_cmos_sensor_28(0x0004);
	Day  = read_cmos_sensor_28(0x0005);
	awb_rg_msb   = read_cmos_sensor_28(0x0006);
	awb_rg_lsb   = read_cmos_sensor_28(0x0007);
	awb_bg_msb   = read_cmos_sensor_28(0x0008);
	awb_bg_lsb   = read_cmos_sensor_28(0x0009);
	awb_GbGr_msb = read_cmos_sensor_28(0x000A);
	awb_GbGr_lsb = read_cmos_sensor_28(0x000B);
	
	S5K3L2_OTP_Infor.MID=MID;
	S5K3L2_OTP_Infor.Year=Year;
	S5K3L2_OTP_Infor.Month=Month;
	S5K3L2_OTP_Infor.Day=Day;
	S5K3L2_OTP_Infor.Lens_id=Lens_id;
	
	S5K3L2_OTP_Infor.AWB_RG=(awb_rg_msb<<8)|(awb_rg_lsb&0XFF);
	S5K3L2_OTP_Infor.AWB_BG=(awb_bg_msb<<8)|(awb_bg_lsb&0XFF);
	S5K3L2_OTP_Infor.AWB_GbGr=(awb_GbGr_msb<<8)|(awb_GbGr_lsb&0XFF);


  printk("S5K3L2 Module Information \n");
	printk("S5K3L2 ModuleID=%02x \n",MID);
	printk("S5K3L2 AWB_RG_MSB=%X ,AWB_RG_LSB=%X\n",awb_rg_msb,awb_rg_lsb);
	printk("S5K3L2 AWB_BG_MSB=%X ,AWB_BG_LSB=%X\n",awb_bg_msb,awb_bg_lsb);
	
	printk("S5K3L2 AWB_RG=%X \n",S5K3L2_OTP_Infor.AWB_RG);
	printk("S5K3L2 AWB_BG=%X \n",S5K3L2_OTP_Infor.AWB_BG);
	printk("S5K3L2 AWB_G=%X \n",S5K3L2_OTP_Infor.AWB_GbGr);
	
	return 1;
}

BYTE S5K3L2_Get_OTP_Info(BYTE Page)
{
    BYTE Flag_AWB = 0x00;
    
    printk("S5K3L2 Page N0 2: page error \n");
    
    Flag_AWB=read_cmos_sensor_28(0x0000);
		S5K3L2_OTP_Infor.AWB_Flag=Flag_AWB;

		if(Flag_AWB==1)
		{
			S5K3L2_get_otp_date(Page);
		}
		else
			{
				S5K3L2_OTP_Infor.MID=0x45;
				S5K3L2_OTP_Infor.Lens_id=0x00;
				S5K3L2_OTP_Infor.Year=15;
				S5K3L2_OTP_Infor.Month=11;
				S5K3L2_OTP_Infor.Day=25;

				S5K3L2_OTP_Infor.AWB_RG=tRG_Ratio_typical;
				S5K3L2_OTP_Infor.AWB_BG=tBG_Ratio_typical;
				S5K3L2_OTP_Infor.AWB_GbGr=1024;	
			}
		
    printk("S5K3L2 : Flag_AWB=0x%02x \n", Flag_AWB );
	  return Flag_AWB;
}
...
void S5K3L2_MIPI_Algorithm_OTP_AWB(struct S5K3L2_MIPI_otp_struct *otp)
{
	...
	printk("S5K3L2_MIPI_Algorithm_OTP_AWB: R_to_G=%d\n", R_to_G);	
	printk("S5K3L2_MIPI_Algorithm_OTP_AWB: B_to_G=%d\n", B_to_G);	
	...
	printk("S5K3L2_MIPI_Algorithm_OTP_AWB: R_gain=0x%x\n", R_Gain);	
	printk("S5K3L2_MIPI_Algorithm_OTP_AWB: B_gain=0x%x\n", G_Gain);	
	printk("S5K3L2_MIPI_Algorithm_OTP_AWB: G_gain=0x%x\n", B_Gain);

	if(R_Gain>0x0100)
		{
			write_cmos_sensor(0x0210, (R_Gain & 0xff00)>>8);
			write_cmos_sensor(0x0211, (R_Gain & 0x00ff)); //R
		}
		
	if(G_Gain>0x0100)
		{
			write_cmos_sensor(0x020e, (G_Gain & 0xff00)>>8);
			write_cmos_sensor(0x020f, (G_Gain & 0x00ff));//GR
		
			write_cmos_sensor(0x0214, (G_Gain & 0xff00)>>8);
			write_cmos_sensor(0x0215, (G_Gain & 0x00ff));//GB
		}
				
	if(B_Gain>0x0100)
		{
			write_cmos_sensor(0x0212, (B_Gain & 0xff00)>>8);
			write_cmos_sensor(0x0213, (B_Gain & 0x00ff)); //B
		}
		
}

void S5K3L2_OTP_AWB_update(struct S5K3L2_MIPI_otp_struct *otp)
{
	BYTE  flag_AWB = 0;
	printk("S5K3L2_OTP_AWB_update\n");
	flag_AWB =otp->AWB_Flag ;
	if(flag_AWB==1)
	{
		S5K3L2_MIPI_Algorithm_OTP_AWB(otp);
	}
}
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0101, 0x00);  //mirror //0x03-0x00 darren
  write_cmos_sensor1(0x6028, 0xD000);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    LOG_INF("s5k3l2 get_imgsensor_id");
...
static kal_uint32 open(void)
{
    ...
    #if 1 //awb using
    S5K3L2_Get_OTP_Info(1);
  	S5K3L2_OTP_AWB_update(S5K3L2_OTP_Infor_Ptr);
  	#endif
