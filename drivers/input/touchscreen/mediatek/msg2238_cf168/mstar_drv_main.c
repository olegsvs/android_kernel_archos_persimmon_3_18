...
extern unsigned int dw_version;
...
#if 0
//static struct proc_dir_entry *msg2238_config_proc = NULL;
//#define MSG2238_CONFIG_PROC_FILE     "msg2238_config"
static const struct file_operations msg_upgrade_proc_fops = { 
    .write = DrvMainProcfsCustomerFirmwareVersionWrite,
    .read = DrvMainProcfsCustomerFirmwareVersionRead,
};
#endif
...
static s32 _DrvMainCreateProcfsDirEntry(void)
{
    ...
    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
	printk("wilson msg2238 line:%d\n", __LINE__);
	printk("wilson msg2238 dw_version:%d, line:%d\n", dw_version, __LINE__);
	tpversion_class = class_create(THIS_MODULE,"dw-mtk-tpd");
	if(IS_ERR(tpversion_class))
	{
		printk("wilson create tp version class failed.\n");
	}
	tpversion_dev = device_create(tpversion_class,NULL,0,NULL,"device");
	if(IS_ERR(tpversion_dev))
	{
		printk("wilson create tp version device failed.\n");
	}
...
#if 0
    msg2238_config_proc = proc_create(MSG2238_CONFIG_PROC_FILE, 0660, NULL, &msg_upgrade_proc_fops);
    if (msg2238_config_proc == NULL)
    {
        printk("create_proc_entry %s failed\n", MSG2238_CONFIG_PROC_FILE);
    }
#endif
