...
static int tpd_keys_factory[TPD_KEY_COUNT] =TPD_KEYS_FACTORY;// add by major for factory keys test
...
//add by major for gesture wakeup 
...
//add by major for gesture switch

static struct class *gesture_switch_class;
static bool TP_gesture_Switch = false;
static ssize_t enable_gesture_show(struct class *dev, struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%s:%d\n", "gesenable",TP_gesture_Switch);
}

static ssize_t enable_gesture_store(struct class *class, struct class_attribute *attr, const char *buf, size_t count)
{
	unsigned long state;
	ssize_t ret = -EINVAL;
...
static CLASS_ATTR(gesenable, 0664, enable_gesture_show, enable_gesture_store);
//add
...
static ssize_t gtp_version_proc_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	char *p;
...
static const struct file_operations gtp_version_proc_ops = {
    .owner = THIS_MODULE,
    .read = gtp_version_proc_read,
};

static void gtp_version_proc(void)
{
    struct proc_dir_entry *gtp_version_prEntry = NULL;	
	gtp_version_prEntry = proc_create("ctp_version", 0, NULL, &gtp_version_proc_ops);
    if (gtp_version_prEntry == NULL) {
        GTP_INFO("add /proc/ctp_version,entry fail \n");
    }
}
...
static int tpd_registration(void *client)
{
    ...
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_A);
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_B);
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_C);
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_D);
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_E);
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_G);
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_H);
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_M);
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_O);
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_Q);
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_S);
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_V);
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_W);
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_Y);
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_Z);
    ...
    input_set_capability(tpd->dev, EV_KEY, KEY_SDE_ENTER);
#endif

//add by major for gesture switch
	gesture_switch_class = class_create(THIS_MODULE, "syna");
	ret = class_create_file(gesture_switch_class, &class_attr_gesenable);
	if (ret) {
		...
		if (DOZE_ENABLED == doze_status) 
		{
			...
			{ //add by major for gesture switch 
		                ...
		                )
		            {
		                if (doze_buf[2] != 0x5E)
		                {
		                    GTP_ERROR("Wakeup by gesture(%c), light up the screen!", doze_buf[2]);
		                }
		                else
		                {
		                    GTP_ERROR("Wakeup by gesture(^), light up the screen!");
		                }
						//doze_status = DOZE_WAKEUP;
		                if(doze_buf[2] == 'a')
		                {
		                    input_report_key(tpd->dev, KEY_SDE_A, 1);
		                    input_sync(tpd->dev);
		                    input_report_key(tpd->dev, KEY_SDE_A, 0);
		                    input_sync(tpd->dev);
		                }   
		                else if(doze_buf[2] == 'b')
		                {
		                    input_report_key(tpd->dev, KEY_SDE_B, 1);
		                    input_sync(tpd->dev);
		                    input_report_key(tpd->dev, KEY_SDE_B, 0);
		                    input_sync(tpd->dev);
		                }   
		                else if(doze_buf[2] == 'c')
		                {
		                    input_report_key(tpd->dev, KEY_SDE_C, 1);
		                    input_sync(tpd->dev);
		                    input_report_key(tpd->dev, KEY_SDE_C, 0);
		                    input_sync(tpd->dev);
		                }   
		                else if(doze_buf[2] == 'd')
		                {
		                    input_report_key(tpd->dev, KEY_SDE_D, 1);
		                    input_sync(tpd->dev);
		                    input_report_key(tpd->dev, KEY_SDE_D, 0);
		                    input_sync(tpd->dev);
		                }   
		                ...
		                else if(doze_buf[2] == 'g')
		                {
		                    input_report_key(tpd->dev, KEY_SDE_G, 1);
		                    input_sync(tpd->dev);
		                    input_report_key(tpd->dev, KEY_SDE_G, 0);
		                    input_sync(tpd->dev);
		                }   
		                else if(doze_buf[2] == 'h')
		                {
		                    input_report_key(tpd->dev, KEY_SDE_H, 1);
		                    input_sync(tpd->dev);
		                    input_report_key(tpd->dev, KEY_SDE_H, 0);
		                    input_sync(tpd->dev);
		                }   
		                ...
		                else if(doze_buf[2] == 'q')
		                {
		                    input_report_key(tpd->dev, KEY_SDE_Q, 1);
		                    input_sync(tpd->dev);
		                    input_report_key(tpd->dev, KEY_SDE_Q, 0);
		                    input_sync(tpd->dev);
		                }   
		                ...
		                else if(doze_buf[2] == 'y')
		                {
		                    input_report_key(tpd->dev, KEY_SDE_Y, 1);
		                    input_sync(tpd->dev);
		                    input_report_key(tpd->dev, KEY_SDE_Y, 0);
		                    input_sync(tpd->dev);
		                }                      
		                ...
		                (doze_buf[2] == 0xAB) || (doze_buf[2] == 0xBA) )
		            {
		                ...
		                GTP_ERROR("%s slide to light up the screen!", direction[type]);
		                //doze_status = DOZE_WAKEUP;
						if(doze_buf[2] == 0xAA)
		                {
		            ...
		            {
		                GTP_ERROR("Double click to light up the screen!");
		                //doze_status = DOZE_WAKEUP;
			...
			goto exit_work_func; //add end by major for gesture wakeup 
		}
#endif
...
#ifdef TPD_HAVE_BUTTON
	if (FACTORY_BOOT == get_boot_mode()) //add by major for factory keycode
	{
    	tpd_button_setting(TPD_KEY_COUNT, tpd_keys_factory, tpd_keys_dim_local);// initialize tpd button data
	}
	else
	{
...
static s8 gtp_wakeup_sleep(struct i2c_client *client)
{
	...
	if(true == TP_gesture_Switch){
		...
		{
			GTP_ERROR("power wakeup, reset guitar");
			
		}
		else
		{
			GTP_ERROR("slide(double click) wakeup, no reset guitar");
		}
...
static void tpd_suspend(struct device *h)
{
	...
	tpd_enter_doze();
#elif GTP_SLIDE_WAKEUP
	if (true == TP_gesture_Switch)//add by major for gesture switch
	{
		ret = gtp_enter_doze(i2c_client_point);
	}else
	{
...
#if GTP_SLIDE_WAKEUP
	 tpd_halt = 0; //add by major for gesture switch
	 if (true == TP_gesture_Switch)
	 {
	 	doze_status = DOZE_DISABLED;
	 }
