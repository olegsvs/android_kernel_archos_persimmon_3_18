...
static s32 _gMutualICDeltaC_2[MUTUAL_IC_MAX_MUTUAL_NUM] = {0};
...
static s32 _DrvMpTestItoOpenTestMsg28xxOpenJudge(u16 nItemID, s8 pNormalTestResult[][2], u16 pNormalTestResultCheck[][13]/*, u16 nDriOpening*/)
{
	...
	for (i = 0; i < nColumnNum; i ++)
	{
		DBG(&g_I2cClient->dev, "*** Msg28xx Open Test# OpenJudge Line=%d ***\n", _gMutualICResult[i * _gMutualICDriveLineNum + nRowNum]);
		if(_gMutualICResult[i * _gMutualICDriveLineNum + nRowNum] !=-32000)
		{
			DBG(&g_I2cClient->dev, "*** Msg28xx Open Test# OpenJudge Line2=%d ***\n", _gMutualICResult[i * _gMutualICDriveLineNum + nRowNum]);
			if(0==_DrvMpTestMutualICCheckValueInRange(_gMutualICResult[i * _gMutualICDriveLineNum + nRowNum], (s32)(1500) , (s32)(600) ))
			{
				_gMutualICTestFailChannel[i * _gMutualICDriveLineNum + nRowNum] = 1;
				_gTestFailChannelCount ++; 
				nRetVal = -1;
			}
		}
...
static s32 _DrvMpTestMsg28xxItoShortTest(u8 nItemID)
{
   ...
   // _DrvMpTestItoShortTestMsg28xxAndChangeCDtime(0x007E, 0x001F);
     _DrvMpTestItoShortTestMsg28xxAndChangeCDtime(0x007E, 0x0006);     //sway
  ...
  //  RegSet16BitValue(0x150C, 0x80A2); //bit15 //AE:3.5v for test
    ...
    if (_DrvMpTestItoShortTestMsg28xxGetValueR(_gMutualICDeltaC_2)<0)
    {
        DBG(&g_I2cClient->dev, "*** Msg28xx Short Test# GetValueR failed! ***\n");      
        return -1;
    }
    _DrvMpTestMutualICDebugShowArray(_gMutualICDeltaC_2, 128, -32, 10, 8);


	 for (i = 0; i < 65; i++) // 13 AFE * 5 subframe
    {
    	if ((_gMutualICDeltaC_2[i] < MUTUAL_IC_IIR_MAX) && (_gMutualICDeltaC[i] < MUTUAL_IC_IIR_MAX))
    		_gMutualICDeltaC[i] = _gMutualICDeltaC_2[i] - _gMutualICDeltaC[i];
    	else
    		_gMutualICDeltaC[i] = _gMutualICDeltaC_2[i];
