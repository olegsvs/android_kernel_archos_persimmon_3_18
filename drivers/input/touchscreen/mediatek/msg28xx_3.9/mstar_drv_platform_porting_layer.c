...
const int g_TpVirtualKey[] = {TOUCH_KEY_BACK, TOUCH_KEY_HOME, TOUCH_KEY_APP_SWITCH, TOUCH_KEY_SEARCH};
const int g_TpVirtualKey_factory[] = {TOUCH_KEY_BACK, TOUCH_KEY_HOME, TOUCH_KEY_MENU, TOUCH_KEY_SEARCH};//add by major for factory key test
...
#else
    input_set_capability(g_InputDevice, EV_KEY, KEY_POWER);
    input_set_capability(g_InputDevice, EV_KEY, KEY_SDE_C);
    input_set_capability(g_InputDevice, EV_KEY, KEY_SDE_E);
    input_set_capability(g_InputDevice, EV_KEY, KEY_SDE_M);
    input_set_capability(g_InputDevice, EV_KEY, KEY_SDE_O);
    input_set_capability(g_InputDevice, EV_KEY, KEY_SDE_S);
    input_set_capability(g_InputDevice, EV_KEY, KEY_SDE_V);
    input_set_capability(g_InputDevice, EV_KEY, KEY_SDE_W);
    input_set_capability(g_InputDevice, EV_KEY, KEY_SDE_Z);
    input_set_capability(g_InputDevice, EV_KEY, KEY_SDE_UP);
    input_set_capability(g_InputDevice, EV_KEY, KEY_SDE_DOWN);
    input_set_capability(g_InputDevice, EV_KEY, KEY_SDE_LEFT);
    input_set_capability(g_InputDevice, EV_KEY, KEY_SDE_RIGHT);
    input_set_capability(g_InputDevice, EV_KEY, KEY_SDE_ENTER);
#endif
#endif //CONFIG_ENABLE_GESTURE_WAKEUP
...
*/
...
#ifdef CONFIG_TP_HAVE_KEY
    {
        ...
        for (i = 0; i < MAX_KEY_NUM; i ++)
        {
			...
			{
            	input_set_capability(g_InputDevice, EV_KEY, g_TpVirtualKey_factory[i]);
			}
			else
			{
            	input_set_capability(g_InputDevice, EV_KEY, g_TpVirtualKey[i]);
