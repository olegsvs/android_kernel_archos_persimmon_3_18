...
static int gesture_flag = 0;//FT xinan
unsigned short coordinate_x[255];
unsigned short coordinate_y[255];
...
#endif
//add by major for ctp proc version 
static char tpversion[20] = {0x00};
static char tpvendor[20] ={0x00};
u8 ctp_ft_fw_ver =0;
...
static bool TP_doubleclick_Switch = false;
static ssize_t enable_ft5336_doubleclick_show(struct class *dev, struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%s:%d\n", "TP_doubleclick_Switch",TP_doubleclick_Switch);
}
static ssize_t enable_ft5336_doubleclick_store(struct class *class, struct class_attribute *attr, const char *buf, size_t count)
{
	unsigned long state;
	ssize_t ret = -EINVAL;
	...
	else
	{
		if(state==true)
		{
			TP_doubleclick_Switch=true;				
		}
		else	
		{
			TP_doubleclick_Switch=false; 
		}						
	}	
	return count;
}
...
static CLASS_ATTR(gesenable, 0664, enable_ft5336_show, enable_ft5336_store);
static CLASS_ATTR(dcenable, 0664, enable_ft5336_doubleclick_show, enable_ft5336_doubleclick_store);
...
#endif
static unsigned short force[]= {0, 0xBA, I2C_CLIENT_END};
...
static const struct of_device_id ft5x0x_gpio_dt_match[] = {
...
static int of_get_ft5x0x_platform_data(struct device *dev)
{
	...
	tpd_rst_gpio_number = 62;//of_get_named_gpio(dev->of_node, "state_rst_output0", 0);
	tpd_int_gpio_number = 10;//of_get_named_gpio(dev->of_node, "state_eint_as_int", 0);
  ...
  */
	TPD_DMESG("g_vproc_rst_gpio_number %d\n", tpd_rst_gpio_number);
	TPD_DMESG("g_vproc_int_gpio_number %d\n", tpd_int_gpio_number);
	return 0;
}
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
	...
	printk(KERN_ERR "Number of touch points = %d\n", cinfo->count);
...
static int fts_read_Gestruedata(void)
{
    ...
    /* FW */
     ...
     {
	     printk( "fts_read_Gestruedata->CHIP_ID==0x54!\n");
     		 gestrue_id = buf[0];
	...
	do {
		...
		TPD_DEBUG("touch_event_handler start\n");
		    ...
		    //   i2c_smbus_read_i2c_block_data(i2c_client, 0xd0, 1, &state);
		     //  printk("kaka tpd fts_read_Gestruedata state=%d\n",state);
		   //  if(state ==1)
		   if(gesture_flag ==1)
		     {
			     printk("read gesture data!\n");
		     		gesture_flag =0;
...
void focaltech_get_upgrade_array(void)
{
	...
	i2c_smbus_read_i2c_block_data(i2c_client, 0x54, 1, &chip_id);
	...
	{
			i = (sizeof(fts_updateinfo)/sizeof(struct Upgrade_Info) - 1);
		memcpy(&fts_updateinfo_curr, &fts_updateinfo[i], sizeof(struct Upgrade_Info));
	}
}
...
static int tpd_irq_registration(void)
{
	...
	TPD_DMESG("mtk_tpd: tpd_irq_registration ft5x0x Touch_irq %d\n", touch_irq);
	return 0;
}
//add by major for proc version 
...
static ssize_t ctpvendor_show(struct device* dev,
                                struct device_attribute* attr,char* buf)
{
     printk("major ..%s %d\n",__func__,__LINE__);
     return sprintf(buf,"%s\n",tpvendor);
}
 static DEVICE_ATTR(version, S_IRUGO | S_IWUSR, tpversion_show, NULL);
 static DEVICE_ATTR(ctpvendor, S_IRUGO | S_IWUSR, ctpvendor_show, NULL);
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int retval = TPD_OK;
	u8 report_rate = 0;
	...
	struct class* tpversion_class = NULL;
	struct device* tpversion_dev = NULL;
	i2c_client = client;
...
reset_proc:
	if ((i2c_smbus_read_i2c_block_data(i2c_client, 0x00, 1, &data)) < 0) {
		...
		TPD_DMESG(TPD_DEVICE " i2c_smbus_read_i2c_block_data 0x00 : %d\n", data);
	...
	TPD_DMESG(TPD_DEVICE " i2c_smbus_read_i2c_block_data 0x88 : %d\n", data);
	/* tpd_load_status = 1; */
	...
	retval = class_create_file(ft5336_class, &class_attr_gesenable);
	if (retval) {
		printk("ft5336 gensenable create sys interface ERROR\n");
	}
	
	retval = class_create_file(ft5336_class, &class_attr_dcenable);
	if (retval) {
		printk("ft5336 dcenable create sys interface ERROR\n");
	}
//add by yangbo 20140926
	temp = fts_get_fw_ver(client);
	if(0 == temp){
		ctp_ft_fw_ver = 0;//fts_get_fw_ver(client);
	}
	else
	{
		ctp_ft_fw_ver = temp;
	...
	sprintf(tpversion,"%02x", ctp_ft_fw_ver);
	printk(" ctp_ft_fw_ver=0x%x \n",ctp_ft_fw_ver);
	temp = fts_get_vendor_id(client);
	if (temp == 0xA6) //boyi vendor_id =0x6E
	{
		sprintf(tpvendor,"%s","BOYI");
	}
	else if(temp == 0xF0)
	{
		sprintf(tpvendor,"%s","TXD");
	}
	else// ofim vendor_id =0x51
	{
		sprintf(tpvendor,"%s", "NOKNOW");
	}
	printk(" ctp_ft_vendor_id=0x%x \n",temp);

	tpversion_class = class_create(THIS_MODULE,"mtk-tpd");
	if(IS_ERR(tpversion_class))
	{
		TPD_DMESG("create tp version class failed.\n");
	}
	tpversion_dev = device_create(tpversion_class,NULL,0,NULL,"device");
	if(IS_ERR(tpversion_dev))
	{
		TPD_DMESG("create tp version device failed.\n");
	}
	if(device_create_file(tpversion_dev,&dev_attr_version)<0)
	{
		TPD_DMESG("create %s failed!\n",dev_attr_version.attr.name);
	}
	if(device_create_file(tpversion_dev,&dev_attr_ctpvendor)<0)
	{
		TPD_DMESG("create %s failed!\n",dev_attr_ctpvendor.attr.name);
	}
...
//	int retval = TPD_OK;
	...
	if(TP_gesture_Switch)
	{
		TPD_DMESG("TPD enter FTS_GESTRUE\n");
		for(counter = 0;counter < 20;counter ++)
			{
				fts_write_reg(i2c_client, 0xd0, 0x01);
				fts_read_reg(i2c_client, 0xd0, &gesture_temp);
				printk("gesture_temp = %d\n",gesture_temp);
				if(gesture_temp == 1)
					{
					     gesture_flag = 1;
						break;
		...
		if (fts_updateinfo_curr.CHIP_ID==0x54)
		{
			TPD_DMESG("TPD enter CHIP_ID\n");
			//if(TP_doubleclick_Switch){
						fts_write_reg(i2c_client, 0xd1, 0xff);
			//	}else{
			//			fts_write_reg(i2c_client, 0xd1, 0xef);
			//}
