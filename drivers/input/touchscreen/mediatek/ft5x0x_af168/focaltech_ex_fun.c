...
extern struct tpd_device *tpd; //add by major 
...
//	#include "FT5436I_AL890_laibao_V0x15_20140925_app.i"
};  
...
static int tpd_dma_read(struct i2c_client *client, char *writebuf,int writelen, char *readbuf, int readlen)
{
	int i = 0;
	int ret = 0;
	if(writelen!=0)
...
static int tpd_dma_write(struct i2c_client *client, unsigned char *pbt_buf, unsigned int dw_len)
{
	...
	else if((dw_len > 8)&&(NULL != I2CDMABuf_va))
	{
		for (i = 0; i < dw_len; i++)
		...
		client->addr = (client->addr & I2C_MASK_FLAG )| I2C_DMA_FLAG;
	    //client->ext_flag = client->ext_flag | I2C_DMA_FLAG;
	    ret = i2c_master_send(client, (unsigned char *)I2CDMABuf_pa, dw_len);
	    client->addr = client->addr & I2C_MASK_FLAG;
		//ret = i2c_master_send(client, (u8 *)(uintptr_t)I2CDMABuf_pa, dw_len);
	    //client->ext_flag = client->ext_flag & (~I2C_DMA_FLAG);
		return ret;
	}
	return 1;
					...
					const char *buf, size_t count)
{
	...
	{
		num_read_chars = snprintf((char *)buf, FTS_PAGE_SIZE,
...
int fts_create_sysfs(struct i2c_client *client)
{
	...
	I2CDMABuf_va = (u8 *)dma_alloc_coherent(&client->dev, FTS_DMA_BUF_SIZE, &I2CDMABuf_pa, GFP_KERNEL);
	...
	const char __user *buff, size_t len, loff_t *data)
{
	...
	case PROC_READ_REGISTER:
		...
		ret = tpd_byte_write(client, writebuf + 1, writelen);
	...
	case PROC_WRITE_DATA:
		writelen = len - 1;
		ret = tpd_dma_write(client, writebuf + 1, writelen);
...
static ssize_t ft5x0x_debug_read(struct file *filp, 
	char __user *buff, size_t len, loff_t *data)
{
	...
	case PROC_READ_REGISTER:
		...
		readlen = 1;
		ret = tpd_byte_read(client, buf, readlen);
    ...
    case PROC_READ_DATA:
		readlen = len;
		ret = tpd_dma_read(client, NULL, 0, buf, readlen);
		if (ret < 0) {
			dev_err(&client->dev, "%s:read iic error\n", __func__);
			return ret;
		}else
...
int ft5x0x_create_apk_debug_channel(struct i2c_client * client)
{
	...
	//ft5x0x_proc_entry = create_proc_entry(PROC_NAME, 0777, NULL);
	 ft5x0x_proc_entry = proc_create(PROC_NAME, 0666, NULL, &ft5x0x_proc_fops);
