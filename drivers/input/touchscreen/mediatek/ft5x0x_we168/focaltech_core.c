...
//#define TP_READ_VER //yanjun.jiang add, 20160127
#ifdef TP_READ_VER
...
#ifdef TP_READ_VER //yanjun.jiang add, 20160127
static ssize_t get_tp_ver_proc(struct file *f, char __user *buf, size_t count, loff_t *pos)
{
	u8 data = 0;
                                ...
                                struct device_attribute* attr,char* buf)
{
 ...
 static DEVICE_ATTR(version,0664,tpversion_show,NULL);
 static DEVICE_ATTR(ctpvendor,0664,ctpvendor_show,NULL);
...
*/
    ...
    temp = 0;
	fts_read_reg(i2c_client, 0XA6, &temp); // Firmware ID
	
	sprintf(tpversion,"%02x", temp);
	printk(" ctp_ft_fw_ver=0x%x \n",temp);
	temp = 0;
    fts_read_reg(i2c_client, 0XA8, &temp); // Vendor ID
	if (temp == 0x6E) //boyi vendor_id =0x6E
	{
		sprintf(tpvendor,"%s","BOYI");
	}
	else if(temp == 0x6C)
	{
		sprintf(tpvendor,"%s","LONGFEI");
	}
	else// ofim vendor_id =0x51
	{
		sprintf(tpvendor,"%s", "OFILM");
	}
...
//add by major for charger detect 
