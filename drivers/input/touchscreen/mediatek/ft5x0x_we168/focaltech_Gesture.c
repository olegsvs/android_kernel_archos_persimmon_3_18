...
int fts_read_Gestruedata(void)
{
    ...
    buf[0] = 0xd3;
     ...
     if((pointnum * 4 + 8)<255)
	 	 ...
	 	 {
	 	        ret = fts_i2c_read(fts_i2c_client, buf, 1, buf, 255);
	 	        ret = fts_i2c_read(fts_i2c_client, buf, 0, buf+255, 255);
	 	        ret = fts_i2c_read(fts_i2c_client, buf, 0, buf+255*2, (pointnum * 4 + 8)-255*2);
	 	 }
	 	 else if(765<(pointnum * 4 + 8) && (pointnum * 4 + 8)<1020)
	 	 {
	 	        ret = fts_i2c_read(fts_i2c_client, buf, 1, buf, 255);
	 	        ret = fts_i2c_read(fts_i2c_client, buf, 0, buf+255, 255);
	 	        ret = fts_i2c_read(fts_i2c_client, buf, 0, buf+255*2, 255);
	 	        ret = fts_i2c_read(fts_i2c_client, buf, 0, buf+255*3, (pointnum * 4 + 8)-255*3);
	 	 }
    if (ret < 0)
    {
        printk( "%s read touchdata failed.\n", __func__);
